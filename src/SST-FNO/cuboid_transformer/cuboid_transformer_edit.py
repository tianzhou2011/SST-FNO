...
class SpectralConv2d(nn.Module):
    ...
    def forward_fourier(self, x):
        #pdb.set_trace()
        x = rearrange(x, 'b s1 s2 s3 i -> b i s2 s3 s1')
...
class CuboidSelfAttentionLayer(nn.Module):
    ...
    def __init__(self,
        ...
        y_dim = 14
        z_dim = 5
        self.FNO3d = SpectralConv2d(self.dim,self.dim,x_dim,y_dim,z_dim,forecast_ff=None,
                           backcast_ff=None,
                                    fourier_weight=False,
                                    factor=4,
                                    n_ff_layers=2,
                                    ff_weight_norm=True,
                                    layer_norm=False,
...
class StackCuboidSelfAttentionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        # self.FNO3d = SpectralConv2d(in_dim=dim,out_dim=dim,modes_z=mode_size[0],modes_x=mode_size[1],modes_y=mode_size[2],
        #                            forecast_ff=None,
        #                    backcast_ff=None,
        #                             fourier_weight=False,
        #                             factor=4,
    ...
    def forward(self, x, global_vectors=None):
        if self.use_inter_ffn:
            ...
            if self.use_global_vector:
                ...
                # if self.checkponit_level>=1 and self.training:
                #     x = x+checkpoint.checkpoint(self.FNO3d,x)
                # else:
                #     x =x+self.FNO3d(x)
                return x, global_vectors
            else:
