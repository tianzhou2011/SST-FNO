...
from earthformer.layers.SpatioTemporalLSTMCell import SpatioTemporalLSTMCell
...
class ReversiblePredictiveModule(nn.Module):
    ...
    def __init__(self, channels: int, n_layers: int, x_dim:int, y_dim:int):
        ...
        for i in range(n_layers):
            self.ConvRNN.append(
                SpatioTemporalLSTMCell(channels, channels, x_dim,y_dim)
            )
