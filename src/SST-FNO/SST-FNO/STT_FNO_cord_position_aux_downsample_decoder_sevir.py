...
class FTSNO_w_gn_bottom_cord_position_aux_downsample_decoder_sevir_Model(nn.Module):
    """
    Autoformer is the first method to achieve the series-wise connection,
    with inherent O(LlogL) complexity
    """
    def __init__(self, n_layers, d_model,total_length,input_length,enc_in,enc_out,modes_x,modes_y,modes_z,factor,n_ff_layers,x_dim,y_dim,z_dim,pre,pre_dim,pad_x,pad_y,pad_z,dropout,downsample_rate=6):
        super(FTSNO_w_gn_bottom_cord_position_aux_downsample_decoder_sevir_Model, self).__init__()        
        ...
        self.prefactor = FNOblock_ST(hidden_features=pre_dim*self.enc_in,n_layers=n_layers,modes_x=int(modes_x/3),modes_y=int(modes_y/3),modes_z=modes_z,x_dim=int(x_dim/3),y_dim=int(y_dim/3),z_dim=z_dim)
        self.preblock = FNOblock(hidden_features=self.in_dim,n_layers=n_layers,modes_x=int(modes_x/self.downsample_rate),modes_y=int(modes_y/self.downsample_rate),modes_z=modes_z,x_dim=int(x_dim/self.downsample_rate),y_dim=int(y_dim/self.downsample_rate),z_dim=z_dim)
        self.midblock_down = FNOblock(hidden_features=2*self.in_dim,n_layers=n_layers,modes_x=int(modes_x/(2*self.downsample_rate)),modes_y=int(modes_y/(2*self.downsample_rate)),modes_z=modes_z,x_dim=int(x_dim/(2*self.downsample_rate)),y_dim=int(y_dim/(2*self.downsample_rate)),z_dim=z_dim)
        self.endblock_down = FNOblock(hidden_features=4*self.in_dim,n_layers=n_layers,modes_x=int(modes_x/(4*self.downsample_rate)),modes_y=int(modes_y/(4*self.downsample_rate)),modes_z=modes_z,x_dim=int(x_dim/(4*self.downsample_rate)),y_dim=int(y_dim/(4*self.downsample_rate)),z_dim=z_dim)
        self.endblock_up = FNOblock(hidden_features=2*self.in_dim,n_layers=n_layers,modes_x=int(modes_x/(2*self.downsample_rate)),modes_y=int(modes_y/(2*self.downsample_rate)),modes_z=modes_z,x_dim=int(x_dim/(2*self.downsample_rate)),y_dim=int(y_dim/(2*self.downsample_rate)),z_dim=z_dim)
        self.midblock_up = FNOblock(hidden_features=self.in_dim,n_layers=n_layers,modes_x=int(modes_x/self.downsample_rate),modes_y=int(modes_y/self.downsample_rate),modes_z=modes_z,x_dim=int(x_dim/self.downsample_rate),y_dim=int(y_dim/self.downsample_rate),z_dim=z_dim)
        ...
        self.outblock = FNOblock(hidden_features=self.in_dim,n_layers=n_layers,modes_x=int(modes_x/self.downsample_rate),modes_y=int(modes_y/self.downsample_rate),modes_z=int(self.pred_len/2),x_dim=x_dim,y_dim=y_dim,z_dim=self.pred_len)
        ...
        self.downsample_top0 = DownSampling3D((z_dim,x_dim,y_dim),(z_dim,int(x_dim/3),int(y_dim/3)),pre_dim*self.enc_in,pre_dim*self.enc_in)
        
        self.downsample_top1 = DownSampling3D((z_dim,int(x_dim/3),int(y_dim/3)),(z_dim,int(x_dim/6),int(y_dim/6)),pre_dim*self.enc_in,pre_dim*self.enc_in)
        
        #self.upsample_end0 = Upsample3DLayer(self.in_dim,self.in_dim,(self.z_dim,int(self.x_dim/3),int(self.y_dim/3)))
        self.upsample_end1 = Upsample3DLayer(self.in_dim,self.in_dim,(self.pred_len,int(self.x_dim),int(self.y_dim)))
                                             
        
        self.downsample1 = DownSampling3D((z_dim,int(x_dim/self.downsample_rate),int(y_dim/self.downsample_rate)),(z_dim,int(x_dim/(2*self.downsample_rate)),int(y_dim/(2*self.downsample_rate))),self.in_dim,2*self.in_dim)
        self.downsample2 = DownSampling3D((z_dim,int(x_dim/(2*self.downsample_rate)),int(y_dim/(2*self.downsample_rate))),(z_dim,int(x_dim/(4*self.downsample_rate)),int(y_dim/(4*self.downsample_rate))),2*self.in_dim,4*self.in_dim)

        self.upsample1 = Upsample3DLayer(4*self.in_dim,2*self.in_dim,(self.z_dim,int(self.x_dim/(2*self.downsample_rate)),int(self.y_dim/(2*self.downsample_rate))))
        self.upsample2 = Upsample3DLayer(2*self.in_dim,self.in_dim,(self.z_dim,int(self.x_dim/self.downsample_rate),int(self.y_dim/self.downsample_rate)))
        ...
        self.downsample_enc = DownSampling3D((z_dim,x_dim,y_dim),(z_dim,int(x_dim/6),int(y_dim/6)),self.in_dim,self.in_dim)
        self.downsample_dec = DownSampling3D((self.pred_len,x_dim,y_dim),(self.pred_len,int(x_dim/6),int(y_dim/6)),self.in_dim,self.in_dim)
    ...
    def forward(self, x_enc,verbose=False,mask_tensor=None):
        ...
        output_grid = self.get_grid2(output_shape,x.device)
        ...
        x_pre = self.downsample_top0(x_pre.permute(0,3,1,2,4)).permute(0,2,3,1,4)
        ...
        #x = x[..., :,:,:-self.pad_z, :]
        #x = x[..., :,:,:self.pred_len, :]
        #x = self.upsample_end0(x.permute(0,3,1,2,4)).permute(0,2,3,1,4)
        aux_enc = self.downsample_enc(aux_enc.permute(0,3,1,2,4)).permute(0,2,3,1,4)
        aux_dec = self.downsample_dec(aux_dec.permute(0,3,1,2,4)).permute(0,2,3,1,4)
        x = self.temp_att(aux_dec.permute(0,4,3,1,2),aux_enc.permute(0,4,3,1,2),x.permute(0,4,3,1,2)).permute(0,2,3,4,1)
        x = self.outblock(x.permute(0,2,3,1,4))
        #pdb.set_trace()
        x = self.upsample_end1(x.permute(0,3,1,2,4))
        #
    ...
    def get_grid2(self, shape,device):
        batchsize, size_x, size_y, size_z = shape[0], shape[1], shape[2], shape[3]
        size_x = size_x
        gridx = torch.tensor(np.linspace(0, 1, size_x), dtype=torch.float)
        gridx = gridx.reshape(1, size_x, 1, 1, 1).repeat(
