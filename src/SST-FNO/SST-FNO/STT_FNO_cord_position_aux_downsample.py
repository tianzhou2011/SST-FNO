...
class FTSNO_w_gn_bottom_cord_position_aux_downsample_Model(nn.Module):
    """
    ...
    def __init__(self, n_layers, d_model,total_length,input_length,enc_in,enc_out,modes_x,modes_y,modes_z,factor,n_ff_layers,x_dim,y_dim,z_dim,pre,pre_dim,pad_x,pad_y,pad_z,dropout,input_shape,target_shape,auxiliary_channels):
        super(FTSNO_w_gn_bottom_cord_position_aux_downsample_Model, self).__init__()        
        ...
        self.aux_proj_pre = nn.Linear(2*self.auxiliary_channels+3,pre_dim*self.enc_in)
    ...
    def forward(self, x_enc,aux_enc, aux_dec, verbose=False):
        ...
        aux_enc=aux_enc.permute(0,2,3,1,4)
        aux_enc = nn.ReplicationPad3d((0,0,self.pad_z,0,0,0))(aux_enc)
        #aux_enc = torch.cat((aux_enc, grid.permute(0,2,3,1,4)), dim=-1)
        aux_enc = torch.cat((aux_enc, grid.permute(0,2,3,1,4),aux_dec.permute(0,2,3,1,4).contiguous()), dim=-1)
        
        
        
        
        x_pre = self.in_proj_pre(x)+self.aux_proj_pre(aux_enc)
