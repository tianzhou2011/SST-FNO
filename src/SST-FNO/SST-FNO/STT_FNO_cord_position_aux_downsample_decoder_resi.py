...
class FTSNO_w_gn_bottom_cord_position_aux_downsample_decoder_resi_Model(nn.Module):
    """
    Autoformer is the first method to achieve the series-wise connection,
    with inherent O(LlogL) complexity
    """
    def __init__(self, n_layers, d_model,total_length,input_length,enc_in,enc_out,modes_x,modes_y,modes_z,factor,n_ff_layers,x_dim,y_dim,z_dim,pre,pre_dim,pad_x,pad_y,pad_z,dropout,input_shape,target_shape,auxiliary_channels):
        super(FTSNO_w_gn_bottom_cord_position_aux_downsample_decoder_resi_Model, self).__init__()        
        ...
        self.preblock = FNOblock(hidden_features=self.in_dim,n_layers=n_layers,modes_x=int(modes_x/2),modes_y=int(modes_y/2),modes_z=modes_z,x_dim=int(x_dim/2),y_dim=int(y_dim/2),z_dim=z_dim)
        self.midblock_down = FNOblock(hidden_features=2*self.in_dim,n_layers=n_layers,modes_x=int(modes_x/4),modes_y=int(modes_y/4),modes_z=modes_z,x_dim=int(x_dim/4),y_dim=int(y_dim/4),z_dim=z_dim)
        self.endblock_down = FNOblock(hidden_features=4*self.in_dim,n_layers=n_layers,modes_x=int(modes_x/8),modes_y=int(modes_y/8),modes_z=modes_z,x_dim=int(x_dim/8),y_dim=int(y_dim/8),z_dim=z_dim)
        self.endblock_up = FNOblock(hidden_features=2*self.in_dim,n_layers=n_layers,modes_x=int(modes_x/4),modes_y=int(modes_y/4),modes_z=modes_z,x_dim=int(x_dim/4),y_dim=int(y_dim/4),z_dim=z_dim)
        self.midblock_up = FNOblock(hidden_features=self.in_dim,n_layers=n_layers,modes_x=int(modes_x/2),modes_y=int(modes_y/2),modes_z=modes_z,x_dim=int(x_dim/2),y_dim=int(y_dim/2),z_dim=z_dim)
        ...
        self.outblock = FNOblock(hidden_features=self.in_dim,n_layers=n_layers,modes_x=int(modes_x/2),modes_y=int(modes_y/2),modes_z=int(self.pred_len/2),x_dim=x_dim,y_dim=y_dim,z_dim=self.pred_len)
        ...
        self.downsample1 = DownSampling3D((z_dim,int(x_dim/2),int(y_dim/2)),(z_dim,int(x_dim/4),int(y_dim/4)),self.in_dim,2*self.in_dim)
        self.downsample2 = DownSampling3D((z_dim,int(x_dim/4),int(y_dim/4)),(z_dim,int(x_dim/8),int(y_dim/8)),2*self.in_dim,4*self.in_dim)

        self.upsample1 = Upsample3DLayer(4*self.in_dim,2*self.in_dim,(self.z_dim,int(self.x_dim/4),int(self.y_dim/4)))
        self.upsample2 = Upsample3DLayer(2*self.in_dim,self.in_dim,(self.z_dim,int(self.x_dim/2),int(self.y_dim/2)))
        
        
        self.in_proj_pre = nn.Linear(self.enc_in+3,pre_dim*self.enc_in)
        self.residual_proj = nn.Linear(self.enc_in+3,self.in_dim)

        self.aux_enc_proj_resi = nn.Linear(self.auxiliary_channels,self.in_dim)
        self.aux_enc_proj= nn.Linear(self.auxiliary_channels,self.in_dim)
        self.aux_dec_proj= nn.Linear(self.auxiliary_channels,self.in_dim)
        ...
        self.posEmbed_enc_aux_residual = PosEmbed(self.in_dim,self.z_dim,self.x_dim,self.y_dim)
    ...
    def forward(self, x_enc,aux_enc, aux_dec, verbose=False):
        ...
        aux_enc = aux_enc[:,:self.seq_len,:,:,:].contiguous()
        aux_enc=aux_enc.permute(0,2,3,1,4)  ### b,h,w,t,c
        aux_dec=aux_dec.contiguous().permute(0,2,3,1,4)
        
        aux_enc_resi = self.aux_enc_proj_resi(aux_enc)
        ...
        aux_enc_resi = self.posEmbed_enc_aux_residual(aux_enc_resi.permute(0,3,1,2,4)).permute(0,2,3,1,4)

        aux_dec = self.posEmbed_dec_aux(aux_dec.permute(0,3,1,2,4)).permute(0,2,3,1,4)
        
        x_residual = self.residual_proj(x).permute(0,3,1,2,4)
        ...
        aux_enc= torch.cat((aux_enc,aux_enc_resi),dim=-2)
